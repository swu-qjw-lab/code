...
class BaseDataSets(Dataset):
    #构造函数的主要任务是根据指定的 split 从相应的文件中读取样本列表，
    #并且如果指定了 num，则只保留列表中的前 num 个样本。
...
class RandomGenerator(object):
    ...
    def __call__(self, sample):#RandomGenerator 的实例可以像函数一样被调用
        ...
        x, y = image.shape
        image = zoom(image, (self.output_size[0] / x, self.output_size[1] / y), order=0)#使用 zoom 函数调整图像和标签的大小以匹配 output_size 的尺寸。
        label = zoom(label, (self.output_size[0] / x, self.output_size[1] / y), order=0)#zoom 函数的 order=0 参数表示使用最近邻插值
        image = torch.from_numpy(image.astype(np.float32)).unsqueeze(0)#将_numpy_数组转换为 PyTorch 张量，并添加一个新的维度，通常是用于表示批次大小的。
...
class TwoStreamBatchSampler(Sampler):#采样器负责从两个不同的索引流创建批次。
    """ 返回的是一个生成器(generator)，
    这个生成器在每次迭代时会生成一个包含来自主要(primary)和
    次要(secondary)索引集的批次(batch)。每个批次包含的是索引(index)的列表，
    这些索引指向您的数据集中的具体数据。 """
    ...
    def __iter__(self):#返回一个迭代器，提供循环时的批次索引。
