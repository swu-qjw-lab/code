...
if __name__ == "__main__":
    # Argument parsing
    parser = argparse.ArgumentParser(description='MIAM Training Script')
    # Paths
    ...
    parser.add_argument('--patch_size', type=list, default=[32, 16, 8])
    # Seed
    ...
    train_dataset = download.load_dataset(args.benchmark, args.datapath, args.thres, device, 'trn', args.augmentation,
                                          args.feature_size)
    val_dataset = download.load_dataset(args.benchmark, args.datapath, args.thres, device, 'val', args.augmentation,
                                        args.feature_size)
    ...
    model = MIAM(
        ...
        hyperpixel_ids=parse_list(args.hyperpixel), freeze=args.freeze,patch_size =args.patch_size )
    ...
    optimizer = optim.AdamW(
        [{'params': param_model, 'lr': args.lr}, {'params': param_backbone, 'lr': args.lr_backbone}],
